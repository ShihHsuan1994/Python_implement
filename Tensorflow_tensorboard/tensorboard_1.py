...
with tf.name_scope('inputs'): #tensorboard 命名
    xs = tf.placeholder(tf.float32, [None, 1], 'x_input') # None 代表輸入多少都可以，
    ys = tf.placeholder(tf.float32, [None, 1], 'y_input') # 因為只有輸入一個特徵所以是 1
...
sess = tf.Session()
sess.run(init)
writer = tf.summary.FileWriter("logs/", sess.graph) # 把整個框架load 到一個文件中
